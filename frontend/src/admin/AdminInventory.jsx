...
export default function AdminInventory() {
  ...
  const loadInventory = useCallback(async () => {
    try {
      setLoading(true);
      const res = await inventoryAPI.getGlobalInventory({ page, limit: 20, q: search });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-dark-900">Global Inventory Management</h1>
          <p className="text-dark-500 text-sm">Monitor and manage stock across all stores</p>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y text-dark-700">
              {loading ? (
                <tr>
                  <td colSpan="5" className="px-6 py-8 text-center text-dark-400">Loading inventory...</td>
                </tr>
              ) : inventory.length === 0 ? (
                <tr>
                  <td colSpan="5" className="px-6 py-8 text-center text-dark-400">No inventory records found.</td>
                </tr>
              ) : (
                inventory.map((item) => (
          ...
          <div className="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl">
            ...
            <form onSubmit={handleAdjustStock} className="p-6 space-y-4">
              <div>
                ...
                <p className="text-xs text-dark-500 mt-1">Store: {selectedItem.store?.name}</p>
              </div>
